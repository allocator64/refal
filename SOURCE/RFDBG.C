...
R4: printf("\n E1= (y/n) : ");
  ...
  if( !s_upto && s_from ) s_upto = 2147483647L;
/*==================================*/
#endif
               /*  initialization  */
  printed_step = 0;
...
one_step(ss_st) st *ss_st; {
   ss_st->stop = s_st.step + 1;
   ss_st->step = s_st.step;  /*!!! kras */
AGAIN:
   rfrun(ss_st);
   if(ss_st != &s_st) s_st.step++;
...
AB:   wrfabe("No memory for parameter");
    ...
    for( i=0; i < l_arg ; i++) 
       *(det_table->det_id + i) = convert(*(arg+i)); 
...
get_arg()  {
   for (l_arg=0; ;l_arg++)
      if( (*(arg+l_arg)=convert(*(arg+l_arg)))=='\n' || *(arg+l_arg) == ' '
