/*-------------------------------------*/
/*            file rfrun1.c            */
/*      last edition date: 21.11.86    */
/*-------------------------------------*/
/*   #define PR_LESC    */
/*   #define PRINT_CODE */
#define NULL 0L
#include "refal.def"
extern REFAL refal;     /* external common block REFAL */
...
static int tmmode;          /* timer state */
 /* tmstart; time at the start */
 /* tmstop; time at the end    */
...
rfrun(ast)    st *ast;/* adress of current state table */  {
 /* dynamic area DSA */
 sav *savecr;
...
/* saving part of refal-block */
 ...
 flhead = refal.flhead;
 if (tmmode == 0) goto START;
/*? must be timer's initial value (tmstart) here ?*/
 goto START;
/* interpreter exits */
/* store state       */
DONE:
...
/* state remove from */
EXIT:
 if (tmmode == 0) goto EXIT0;
/*? must be timer's last value (tmstop) here ?*/
EXIT0:
...
/* restore REFAL-block */
 ...
 free(savecr);
 if (tmmode != 0){
                   /*? account refal.tmintv = tmstop - tmstart ?*/
                 };
 return;
...
#ifdef PR_LESC
   printf ("\n cycle cmpr ,symbol = %c",b1->info.infoc);
#endif
